...
function build_gif(msg, callback){
    ...
    // start command
    var command = ['-delay', '50', 'imgs/off.png', '-delay', '50', 'imgs/on.png', '-delay', '50', 'imgs/off.png']; 
    var gif_filename = '/tmp/';

    var space = ['-delay', '20', 'imgs/off.png'];
    ...
    for (var i = 0, len = msg.length; i < len; i++) {
        ...
        if (lc.match(/[a-z]/i)){
            command = command.concat(['-delay', '75', 'imgs/'+lc+'.png']);
            gif_filename += lc;
        }
        else{
    ...
    im.convert(command, 
        function(err, stdout){
            ...
            fs.readFile(gif_filename, function (err, data) {
                ...
                var s3_filename = parseInt(+ new Date()) + '.gif';
